namespace MinotaurLabyrinth
{
    static class Display
    {
        ...
        // Displays the status to the player, including what MyMosterRoom they are in and asks each sense to display
        ...
        private static bool DisplaySenses(Hero hero, Map map)
        {
            ...
            {
                var MyMosterRoom = map.GetMyMosterRoomAtLocation(loc);
                (int lx, int ly) = loc;
                int distanceFromPlayer = Math.Abs(lx - px) + Math.Abs(ly - py);
                if (MyMosterRoom.DisplaySense(hero, distanceFromPlayer)) ++sensedMyMosterRooms;
            }
            return sensedMyMosterRooms > 0;
        }
